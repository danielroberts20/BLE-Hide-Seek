...
async def EnhanceColor(normalized):
    if normalized > 0.04045:
        return math.pow( (normalized + 0.055) / (1.0 + 0.055), 2.4)
    else:
        return normalized / 12.92

async def RGBtoXY(r, g, b):
    ...
    rFinal = await EnhanceColor(rNorm)
    gFinal = await EnhanceColor(gNorm)
    bFinal = await EnhanceColor(bNorm)
...
async def rgb_to_hue(r, g, b):
    x, y = await RGBtoXY(r, g, b)
    return await xy_to_ble(x, y)

async def lerp_color_heatmap(t, color1=(0, 0, 255), color2=(255, 0, 0)):
